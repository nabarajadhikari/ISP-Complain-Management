...
class Ion_auth_model extends CI_Model
{
	 ...
	 * @author Mathew
	 **/
	public function login($identity, $password, $remember=FALSE,$group_id=false)
	{
	...
	**/
	...
	public function register1($username, $password, $email,
$additional_data = array(), $groups = array())
    {
        $this->trigger_events('pre_register');
...
'ion_auth');
        ...
        elseif ($this->identity_column == 'username' &&
$this->username_check($username))
        ...
        //filter out any data passed that doesnt have a matchingcolumn in the users table
        //and merge the set user data and the additional data        
		$user_data =array_merge($this->_filter_data($this->tables['users'],
$additional_data), $data);
        ...
        //add to default group if not already set
        $default_group = $this->where('name',
$this->config->item('default_group', 'ion_auth'))->group()->row();
        if ((isset($default_group->id) && !isset($groups)) ||
(empty($groups) && !in_array($default_group->id, $groups)))
        {
            $this->add_to_group($default_group->id, $id);
        }
