...
//mdie($_POST);
            ...
            if ($this->form_validation->run() == true )
            {
                ...
                $_POST['status']='Pending';
				 $_POST['status_id']=1; 
if($_POST['urgent']==on){ $_POST['urgent']=1;$_POST['status']='Urgent Pending'; $_POST['status_id']=-0; }
...
//mdie($_POST);
    ...
    if(isset($_POST['urgent'])) {$_POST['urgent']=1;$_POST['status']='Urgent Pending';$_POST['status_id']=0;}
else {$_POST['urgent']=0;$_POST['status']='Pending';$_POST['status_id']=1;}
                unset($_POST['id']);
...
$data['urgnt']=$complain->urgent;
	...
	{
	die('herer');
	/* $where=array('status'=>"Urgent Pending");
	 $data['status_id']=0;
	 $where1=array('status'=>"Pending");
	 $data1['status_id']=1;
	  $where2=array('status'=>"Assigned");
	 $data2['status_id']=2;*/
	...
	// $this->complain_model->update_complain($data1,$where1);
	// $this->complain_model->update_complain($data2,$where2);
	// $this->complain_model->update_complain($data3,$where3);
	// $this->complain_model->update_complain($data4,$where4);
	 //mdie($this->complain_model->getcomplains());
	 
	
	}
	// testing for odbc
function put_access($post)
     {
     
        $defdir = FCPATH.'assets/ad';
		mdie($defdir);
        $dbq    =    FCPATH.'assets/ad/db.mdb';
	// $defdir = "C://wamp/www/test12";
     //   $dbq    =    "C://wamp/www/test12/db.mdb";
        if    (!file_exists($dbq)) { die("Database file $dbq does not exist"); }

         $dsn = "DRIVER=Microsoft Access Driver (*.mdb);UID=;FIL=MS Access;DriverId=25;DefaultDir=".$defdir.";DBQ=".$dbq;
        ...
        $sql="insert into Table1(name,roll_no) values('".$post['name']."','".$post['roll']."')";
     ...
     function read_excel(){
                ...
                $this->spreadsheet_excel_reader->read(FCPATH.'assets/ad/bb.xls'); // relative path to .xls that was uploaded earlier
                ...
                if($success)echo 'success, '.'row '.$s_msg.' successfully added';
                if($error)echo 'error, Error adding following rows:'.$e_msg; 
     }
