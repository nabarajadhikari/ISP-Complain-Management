...
<tr>
 ...
 <?php foreach($blogs as $blog):?>
                            <tr <?php if($blog->status=='Pending') echo 'id="fail"';elseif($blog->status=='Finished') echo  'id="success"';elseif($blog->status=='Processing')echo 'id="processing"';elseif($blog->status=='Assigned') echo 'id="warning"';?>>
                                    ...
                                    <td><a href="#" onclick="show_div('<?php echo $blog->id?>','<?php echo $blog->status?>')"><?php if($blog->status=='Pending') echo 'Assign';elseif($blog->status=='Processing')echo 'Finish';elseif($blog->status=='Assigned') echo 'Processing';?></a> <?php if($blog->status=='Processing') {?><a href="#" onclick="show_div('<?php echo $blog->id?>','Assigned')">Update-Comments</a><?php }?><?php if($this->session->userdata('user_group')!='technician'){?><a href="<?php echo site_url('user/edit_complain/'.$blog->id)?>"><img src="<?php echo ASPATH ; ?>/img/edit.png" alt="Edit" /></a><a href="<?php echo site_url('user/delete_complain/'.$blog->id)?>"><img src="<?php echo ASPATH ; ?>/img/cancel.png" alt="Edit" /></a> <?php if($blog->status=='Pending' || $blog->status=='Assigned') {?><a href="#" onclick="show_div('<?php echo $blog->id?>','Processing')">Finish</a><?php }?><?php }?></td>
