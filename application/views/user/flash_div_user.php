...
elseif($blog->status=='Urgent Pending')
{
echo 'id="urgent"';
...
elseif($blog->status=='Finished'){ echo  'id="success"';continue;}elseif($blog->status=='Processing')echo 'id="processing"';elseif($blog->status=='Assigned') echo 'id="warning"';?> style="border:3px solid #c3c3c3 !important;margin:2px;">
                              <td  width="35%" style="padding:5px;"> 

                                   <div style="text-align:center;font-size:16px;float:left;"><b><?php echo $blog->complain_title;
								   if($blog->urgent==1)
									{?>
                             ...
                             <td  width="15%" style="padding:5px;">
                              ...
                              <td  width="35%"  <?php if($blog->status=='Urgent Pending'){ ?> style="padding:5px;background-image:url("<?php echo base_url();?>assets/uploads/urgent.gif");background-repeat:no-repeat;" <?php } else {?> style="padding:5px;" <?php } ?> >
                                ...
                                <?php if($blog->status=="Finished"){?><span><?php echo $blog->finished_date;?></span><br/><?php }?>
                                    ...
                                    <span><?php if($blog->status=='Pending' || $blog->status=='Urgent Pending' || $blog->status=='Assigned') {?><a href="#" onclick="show_div('<?php echo $blog->id?>','Processing')">Finish</a><?php }?></span>
